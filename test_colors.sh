...
# Fixed show_menu_option function
show_menu_option() {
    local num="$1"
    local desc="$2"
    # The color codes are already embedded in the desc string, so just use echo -e for proper interpretation
    echo -e "  ${GREEN}$(printf '%-3s' "$num")${NC}. $desc"
}

echo "Testing the fixed show_menu_option function:"
